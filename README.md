...
Для скачивания данных [FRED](https://fred.stlouisfed.org/) используется ключ API, который можно получить после регистрации на сайте. Подробнее на [CRAN](https://cran.r-project.org/web/packages/fredr/vignettes/fredr.html#authentication) и [FRED](https://research.stlouisfed.org/docs/api/api_key.html). Без ключа данные с FRED скачиваться не будут.
